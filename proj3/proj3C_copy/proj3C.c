...
char *getName(char *string){	
	if(getBlood(string) == "AB"){
		string += 5;
	}
	else if(getBlood(string) != "AB"){
		string += 4;
	}
...
int SurgeonIsMatch(char **types, char *surgeon, Queue **donors, Queue **recipients){
	...
	while(getline(&buffer, &size, f) > 0){
		...
		for(int i = 0; i < BLOOD_TYPES; i++){
			if(type == 'S'){
				...
				if(SurgeonIsMatch(types, name, donors, recipients) == 0){ 
					enqueue(name, surgeons);
					break;	
					}
				else{SurgeonIsMatch(types, name, donors, recipients);}
				
			}	
							
			else if(type == 'R'){
				if(isDonorToRecipient(types[i], getBlood(string))){
					if(donors[i]->population > 0 && surgeons->population > 0){
						char *don = dequeue(donors[i]); 
						char *sur = dequeue(surgeons);
      						printf("MATCH: %s donates to %s via Dr. %s\n", 
							don, getName(string), sur); 
						break;
					}
					else{
						char *repName = getName(string);
						enqueue(getName(string), recipients[i]);
						break;
					}
				...
				if(isDonorToRecipient(getBlood(string), types[i])){
					if(recipients[i]->population > 0 && surgeons->population > 0){
						char *rep = dequeue(recipients[i]); 
						char *sur = dequeue(surgeons); 
						printf("MATCH: %s donates to %s via Dr. %s\n", 
								getName(string), rep, sur);
					}
					else{
						char *donName = getName(string);
				 		enqueue(donName, donors[i]);
						break;
					}
				}
			}
